...
<div class="max-w-2xl mx-auto bg-white rounded-2xl shadow-card border border-gray-100 p-8 mt-8 animate-fade-in">
    <h2 class="text-2xl font-bold mb-6 text-gray-900 flex items-center gap-2">
        <i class="ph-book-open-bold text-blue-600"></i> Edit Buku
    </h2>
    <form action="{{ route('admin.books.update', $book) }}" method="POST" enctype="multipart/form-data" class="space-y-7">
        @csrf
        @method('PUT')
        <div>
            ...
            <input type="text" name="title" id="title" value="{{ old('title', $book->title) }}" required placeholder="Masukkan judul buku" class="block w-full rounded-md border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 shadow-sm transition-smooth">
        ...
        <div>
            ...
            <input type="text" name="author" id="author" value="{{ old('author', $book->author) }}" required placeholder="Nama penulis" class="block w-full rounded-md border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 shadow-sm transition-smooth">
        ...
        <div>
            ...
            <input type="text" name="publisher" id="publisher" value="{{ old('publisher', $book->publisher) }}" placeholder="Nama penerbit" class="block w-full rounded-md border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 shadow-sm transition-smooth">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="publication_year" id="publication_year" value="{{ old('publication_year', $book->publication_year) }}" required min="1000" max="9999" placeholder="Contoh: 2024" class="block w-full rounded-md border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 shadow-sm transition-smooth">
            ...
            <div>
                ...
                <input type="text" name="isbn" id="isbn" value="{{ old('isbn', $book->isbn) }}" required placeholder="Nomor ISBN" class="block w-full rounded-md border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 shadow-sm transition-smooth">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3" placeholder="Deskripsi singkat buku" class="block w-full rounded-md border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 shadow-sm transition-smooth">{{ old('description', $book->description) }}</textarea>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="stock" id="stock" value="{{ old('stock', $book->stock) }}" min="0" required placeholder="Jumlah stok" class="block w-full rounded-md border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 shadow-sm transition-smooth">
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-1">
                <i class="ph-image-bold text-blue-500"></i> Tipe Cover
            </label>
            <div class="flex items-center gap-4">
                <label class="inline-flex items-center gap-1">
                    <input type="radio" name="cover_type" value="url" {{ old('cover_type', $book->cover_url ? 'url' : 'none') == 'url' ? 'checked' : '' }} class="form-radio text-blue-600">
                    <i class="ph-link-bold text-blue-400"></i> <span class="ml-1">URL</span>
                </label>
                <label class="inline-flex items-center gap-1">
                    <input type="radio" name="cover_type" value="file" {{ old('cover_type') == 'file' ? 'checked' : '' }} class="form-radio text-blue-600">
                    <i class="ph-upload-simple-bold text-blue-400"></i> <span class="ml-1">Upload File</span>
                </label>
                <label class="inline-flex items-center gap-1">
                    <input type="radio" name="cover_type" value="none" {{ old('cover_type') == 'none' || !$book->cover_url ? 'checked' : '' }} class="form-radio text-blue-600">
                    <i class="ph-x-bold text-red-400"></i> <span class="ml-1">Tanpa Cover</span>
                </label>
            </div>
            @error('cover_type')<p class="text-red-500 text-xs mt-1 flex items-center gap-1"><i class="ph-warning-bold"></i> {{ $message }}</p>@enderror
        </div>
        <div id="cover-url-input" class="mt-2 transition-smooth" style="display: {{ old('cover_type', $book->cover_url ? 'url' : 'none') == 'url' ? 'block' : 'none' }};">
            ...
            <input type="url" name="cover_url" id="cover_url" value="{{ old('cover_url', $book->cover_url) }}" placeholder="https://..." class="block w-full rounded-md border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 shadow-sm transition-smooth">
        ...
        <div class="mt-4">
            <label class="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-1">
                <i class="ph-image-bold text-blue-400"></i> Preview Cover Saat Ini:
            </label>
            <img src="{{ $book->cover_url }}" alt="Cover Buku" class="w-32 h-44 object-cover rounded shadow-card border">
        </div>
        @endif
        <div class="flex items-center gap-3 mt-8">
            <button type="submit" class="inline-flex items-center px-5 py-2 bg-blue-600 text-white font-semibold rounded-md shadow-card hover:bg-blue-700 transition-smooth gap-2">
                <i class="ph-floppy-disk-bold"></i> Simpan Perubahan
            </button>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const noneRadio = document.querySelector('input[name="cover_type"][value="none"]');
        ...
        noneRadio.addEventListener('change', toggleCoverInput);
        toggleCoverInput();
    });
</script>
@endsection
